...
%% @end
%%--------------------------------------------------------------------
-spec(start_link(ServiceSupRef :: pid(), SupRef :: pid()) ->
    {ok, Pid :: pid()} | ignore | {error, Reason :: term()}).
start_link(ServiceSupRef, SupRef) ->
    io:format("Pyfleet game manager. Start link. ServiceSupRef: ~p, Sup ref: ~p~n", [ServiceSupRef, SupRef]),
    gen_server:start_link({local, ?SERVER}, ?MODULE, [ServiceSupRef, SupRef], []).
...
init([ServiceSupRef, SupRef]) ->
  io:format("Pyfleet game manager init~n", []),
  self() ! init,
  IdQueue = queue:from_list(lists:seq(1, ?MAX_GAME_COUNT * 2)),
  {ok, #state{service_sup_ref = ServiceSupRef, sup_ref = SupRef, game_id_queue = IdQueue}}.
...
handle_call(#pyfleet_join_game{game_id = GameId, player_name = PlayerName, player_ref = PlayerRef}, From, State = #state{running_game_list = RunningGameList}) ->
  io:format("Pyfleet game manager. Player ~p join game~n", [PlayerName]),
  case lists:keytake(GameId, #game_description.id, RunningGameList) of
    {value, GameDescription = #game_description{game_ref = GameRef, player_list = PlayerList}, GameList2} ->
...
handle_info(init, State = #state{service_sup_ref = ServiceSupRef, sup_ref = SupRef}) ->
  io:format("Game manager. Initialization..~n", []),
  {ok, UserSupRef} = create_game_worker_supervisor(SupRef),
  GameManagerRef = self(),
  pyfleet_player_pool_service_sup:start_as_child(ServiceSupRef, GameManagerRef),
