...
%% API
-export([start_link/3, async_login/3, relogin/3]).
...
%% @end
%%--------------------------------------------------------------------
-spec(start_link(ServiceSup :: pid(), SupRef :: pid(), GameManagerPid :: pid()) ->
    {ok, Pid :: pid()} | ignore | {error, Reason :: term()}).
start_link(ServiceSup, SupRef, GameManagerPid) ->
    io:format("Pyfleet player manager. Start link. ServiceSup: ~p, Sup ref: ~p, Game manger pid: ~p~n", [ServiceSup, SupRef, GameManagerPid]),
    gen_server:start_link({local, ?SERVER}, ?MODULE, [ServiceSup, SupRef, GameManagerPid], []).

async_login(PlayerManagerRef, PlayerName, SecurityData) ->
  gen_server:call(PlayerManagerRef, {async_login, PlayerName, SecurityData}).

relogin(PlayerManagerRef, PlayerName, SecurityData) ->
  gen_server:call(PlayerManagerRef, {relogin, PlayerName, SecurityData}).
...
init([ServiceSup, SupRef, GameManagerPid]) ->
  io:format("Pyfleet player manager init~n", []),
  self() ! init,
  {ok, #state{service_sup = ServiceSup, sup_ref = SupRef, game_manager_ref = GameManagerPid}}.
...
handle_call({relogin, User, _SecurityData}, _From, State = #state{logged_in_users = LoggedInUsers}) ->
  case is_logged_in(User, LoggedInUsers) of
    {true, {User, _, PlayerSessionPid}} ->
      {reply, {ok, PlayerSessionPid}, State};
    false ->
      io:format("Async login user ~p~n", [User]),
      {reply, not_logged_in, State}
  end;
handle_call({async_login, User, SecurityData}, {ApiRef, _} = _From, State = #state{user_sup_ref = UserSupRef, user_count = UserCount, max_user_count = MaxUserCount, logged_in_users = LoggedInUsers, game_manager_ref = GameManagerRef}) when UserCount < MaxUserCount ->
  case is_logged_in(User, LoggedInUsers) of
    {true, {User, _, PlayerSessionPid}} ->
      io:format("Logged in~p~n", [User]),
      {reply, {logged_in, PlayerSessionPid}, State};
    false ->
      io:format("Async login user ~p~n", [User]),
      case pyfleet_player:async_login(UserSupRef, self(), ApiRef, GameManagerRef, User, SecurityData) of
        {ok, PlayerSessionRef} ->
          PlayerSessionMonitorRef = erlang:monitor(process, PlayerSessionRef),
          {reply, {ok, PlayerSessionRef}, State#state{user_count = UserCount + 1, logged_in_users = [{User, PlayerSessionMonitorRef, PlayerSessionRef} | LoggedInUsers]}};
        {error, Reason} ->
...
handle_info(init, State = #state{service_sup = ServiceSup, sup_ref = SupRef}) ->
  io:format("Pyfleet player manager. Initialization..~n", []),
  {ok, UserSupRef} = create_player_pool_worker_supervisor(SupRef),
  PlayerManagerRef = self(),
  start_webmachine_socket_server(ServiceSup, PlayerManagerRef),
...
is_logged_in(User, LoggedInUsers) ->
    case lists:keysearch(User, 1, LoggedInUsers) of
        {value, {_User, _SessionRef, _SessionPid} = LoggedIn} ->
            {true, LoggedIn};
        false ->
            false
    end.
...
start_webmachine_socket_server(ServiceSup, PlayerManagerRef) ->
  Child = {webmachine_mochiweb,
    {webmachine_mochiweb, start, [pyfleet_config:web_config(PlayerManagerRef)]},
    permanent, 5000, worker, [mochiweb_socket_server]},
  supervisor:start_child(ServiceSup, Child).
%%++++++++++++++++++++++++++++++++++++++++++++++++++++
